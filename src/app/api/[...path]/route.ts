...
import { handleAuth, requireAdmin } from '@/lib/server/auth';
  ...
  try {
    ...
    if (method === 'POST' && segments[0] === 'issues' && segments[2] === 'participations' && segments.length === 3) {
      ...
      return json((await transaction(async () => {
        const issue = (await issues('i.id=?', [segments[1]], input.clientId))[0]; if (!issue) return notFound();
        if (issue.status === 'resolved') throw new ApiError(409, 'ISSUE_RESOLVED', '이미 해결된 고장이에요. 최신 상태를 확인해 주세요.');
        (await run('INSERT OR IGNORE INTO participations VALUES (?,?,?)', issue.id, input.clientId, iso(Date.now())));
        return { issueId: issue.id, affectedCount: (await issues('i.id=?', [issue.id]))[0].affectedCount, alreadyParticipated: issue.hasParticipated };
      })));
    }
    if (method === 'PATCH' && segments[0] === 'issues' && segments.length === 2) {
      ...
      return json((await transaction(async () => {
        const issue = (await issues('i.id=?', [segments[1]], clientId))[0]; if (!issue) return notFound();
        ...
        (await run('UPDATE issues SET status=?,eta_text=?,updated_at=?,resolved_at=? WHERE id=?', input.status, input.status === 'resolved' ? null : input.etaText || null, now, input.status === 'resolved' ? issue.resolvedAt || now : null, issue.id));
        return { issue: (await issues('i.id=?', [issue.id], clientId))[0] };
      })));
    }
    if (method === 'GET' && path === 'admin/predictions') {
      ...
      return json(await predictions());
    }
    if (method === 'GET' && (path === 'admin/issues' || path === 'overview')) {
      ...
      const list = (await issues('1=1', [], clientId));
      const catalog = (await facilities());
      return json({ issues: list.filter(i => state === 'open' ? i.status !== 'resolved' : i.status === 'resolved').sort(state === 'open' ? sortOpen : sortResolved).map(i => ({ ...i, facility: catalog.find(f => f.id === i.facilityId) })),
    ...
    if (method === 'GET' && segments[0] === 'admin' && segments[1] === 'issues' && segments.length === 3) {
      const issue = (await issues('i.id=?', [segments[2]], clientId))[0]; if (!issue) return notFound();
      ...
      const reports: Report[] = (await all<Row>(`SELECT r.*,p.id AS photo_id,p.mime_type,p.byte_size FROM reports r LEFT JOIN photos p ON p.report_id=r.id WHERE r.issue_id=? ORDER BY r.created_at,r.id`, issue.id))
      ...
      return json({ issue, facility: (await facility(issue.facilityId)), reports });
    }
    if (method === 'GET' && segments[0] === 'uploads' && segments.length === 2) {
      ...
      const row = await one<{ storage_key: string; mime_type: string }>('SELECT storage_key,mime_type FROM photos WHERE id=?', segments[1]);
      if (!row) throw new ApiError(404, 'PHOTO_NOT_FOUND', '사진을 찾을 수 없습니다.');
      const content = (await one<{ content: ArrayBuffer }>('SELECT content FROM photo_contents WHERE photo_id=?', segments[1]));
      if (content) return new Response(new Uint8Array(content.content), { headers: { 'Content-Type': row.mime_type, 'Cache-Control': 'no-store', 'X-Content-Type-Options': 'nosniff' } });
      let bytes: Buffer;
      try { bytes = readFileSync(/* turbopackIgnore: true */ resolve(process.env.UPLOAD_DIR || './data/uploads', row.storage_key)); }
