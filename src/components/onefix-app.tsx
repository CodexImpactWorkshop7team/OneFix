...
type View = 'home' | 'facility' | 'report' | 'admin';
const title = { home: '시설 현황', facility: '시설 상세', report: '고장 신고', admin: '관리자 대시보드' };
...
export default function OneFixApp({ view, facilityId }: { view: View; facilityId?: string }) {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <div className="workspace"><div className="workspace-icon"><Building2 size={19} /></div><div><strong>우리 캠퍼스</strong><span>시설 관리 워크스페이스</span></div></div>
      <div className="nav-label">WORKSPACE</div>
      <nav aria-label="주 메뉴">
        <Link href="/" className={`nav-item ${view !== 'admin' ? 'active' : ''}`}><Building2 size={20} />시설 현황<span className="nav-count">{catalog?.facilities.length || 3}</span></Link>
        <Link href="/admin" className={`nav-item ${view === 'admin' ? 'active' : ''}`}><ClipboardList size={20} />신고 관리<ChevronRight size={16} className="nav-chevron" /></Link>
        <Link href="/admin#predictions" className="nav-item"><BarChart3 size={20} />예방 점검<span className="nav-new">NEW</span></Link>
      </nav>
      <div className="sidebar-tip"><span className="tip-symbol"><QrCode size={25} /></span><strong>시설 앞에서, 바로 확인</strong><p>QR 하나로 고장 신고와<br />처리 현황을 함께 확인해요.</p></div>
    ...
    <div className="main-shell">
      ...
      <main className="main-content">
        ...
        {catalog?.meta.dedupMode === 'mock' && <div className="demo-strip"><Sparkles size={14} /><span>데모 병합 모드 <span className="demo-divider">·</span> 정해진 예시 문장으로 중복 신고를 확인합니다.</span>{catalog.meta.datasetKind === 'synthetic' && <span className="demo-label">샘플 데이터</span>}</div>}
        ...
        {catalog && view === 'home' && admin && <>
          ...
          <div className="facility-grid">{catalog.facilities.map(f => {
            ...
            return <article className="facility-card" key={f.id}><div className="facility-card-top"><div className={`facility-icon kind-${f.kind}`}><FacilityIcon kind={f.kind} size={27} /></div>{lead ? <Badge status={lead.status} /> : <span className="badge status-resolved"><Check size={13} />접수 없음</span>}</div>
              ...
              <div className="facility-current"><span className="small-label">현재 신고</span><strong>{lead ? symptomLabels[lead.symptom] : '현재 접수된 고장이 없습니다'}</strong><p>{lead?.description || '새로운 문제가 있다면 알려 주세요.'}</p></div>
          ...
          <div className="bottom-banner"><div className="banner-icon"><CheckCheck size={28} /></div><div><h3>이미 신고된 문제라면, 한 번의 클릭으로.</h3><p>‘나도 겪고 있어요’를 누르면 영향 인원이 늘어나 관리자에게 전달돼요.</p></div><ArrowUpRight size={24} /></div>
        ...
        {catalog && (view === 'facility' || view === 'report') && detail && <>
          ...
          {view === 'facility' ? <>
            ...
            <div className="detail-layout"><section><div className="section-heading tight"><h2>현재 접수된 고장 <span className="count-tag">{detail.issues.length}</span></h2></div>
              ...
              {!detail.issues.length && <div className="panel empty-state"><div className="success-ring"><Check size={26} /></div><h3>현재 접수된 고장이 없습니다</h3><p>새로운 불편이 있다면 아래에서 알려 주세요.</p><Link className="button primary" href={`/facilities/${facilityId}/report`}><Plus size={18} />고장 신고하기</Link></div>}
            ...
            </section><aside className="detail-aside"><div className="report-prompt"><span className="eyebrow">SOMETHING ELSE?</span><h2>다른 문제가<br />있나요?</h2><p>목록에 없는 고장이라면<br />새로운 신고를 남겨 주세요.</p><Link className="button primary" href={`/facilities/${facilityId}/report`}><Plus size={18} />다른 문제 신고</Link></div><div className="aside-note"><Users size={21} /><h3>같은 문제라면 참여해 주세요</h3><p>별도로 신고하지 않아도 돼요. 함께 겪는 사람 수가 관리자에게 전달됩니다.</p></div></aside></div>
        ...
        <footer className="footer"><span><Wrench size={13} /> OneFix <span className="footer-dot">·</span> 같은 고장, 하나의 신고</span><span>함께 만드는 더 편리한 캠퍼스</span></footer>
      </main>
    </div>
...
function ReportForm({ facility, photos }: { facility: Facility; photos: boolean }) {
  ...
  return <div className="report-layout"><div className="report-intro"><div className="eyebrow">NEW REPORT</div><h1>어떤 불편이<br />있으신가요?</h1><p>증상을 알려 주시면 같은 고장인지<br />확인한 후 접수해 드려요.</p><div className="report-facility"><div className={`facility-icon kind-${facility.kind}`}><FacilityIcon kind={facility.kind} /></div><div><strong>{facility.name}</strong><span>{facility.location}</span></div></div><div className="aside-note"><Sparkles size={22} /><h3>같은 신고는 하나로 모여요</h3><p>이미 접수된 고장과 같은 문제라면 기존 신고에 합쳐집니다.</p></div></div><form onSubmit={submit} className="report-form panel"><fieldset disabled={busy}><label className="field-label">어떤 증상인가요?<span>필수</span></label><div className="symptom-options">{symptoms.map(s => <button key={s} type="button" className={symptom === s ? 'selected' : ''} onClick={() => { setSymptom(s); edited(); }}>{symptomLabels[s]}{symptom === s && <Check size={15} />}</button>)}</div><label className="field-label" htmlFor="description">조금 더 자세히 알려 주세요<span>필수</span></label><textarea id="description" value={description} onChange={e => { setDescription(e.target.value); edited(); }} maxLength={500} rows={5} placeholder="예: 출력 버튼을 눌러도 종이가 안 나와요" required /><div className="input-help"><span>문제가 발생하는 상황을 적어 주세요.</span><span>{description.length} / 500</span></div>
      ...
      {error && <Notice>{error}</Notice>}<div className="form-bottom"><p><ShieldCheck size={15} />입력한 신고는 담당자가 확인합니다.</p><button className="button primary full" disabled={busy || description.trim().length < 5} type="submit">{busy ? <><LoaderCircle size={18} className="spin" />기존 신고를 확인하고 있어요…</> : <>신고 접수하기<ArrowRight size={18} /></>}</button></div></fieldset></form></div>;
}
