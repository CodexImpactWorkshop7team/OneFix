...
export default function QuestionsBoard({ admin = false }: { admin?: boolean }) {
  ...
  const filtered = questions.filter(q => (department === 'all' || department === q.department) &&
    (status === 'all' || (status === 'review' ? q.submissionCount > q.answeredSubmissionCount : status === 'answered' ? Boolean(q.answer) : !q.answer)) &&
    `${q.title} ${q.description} ${q.period} ${q.answer || ''}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase()))
    .sort((a, b) => sort === 'newest' ? b.createdAt.localeCompare(a.createdAt) : sort === 'review' ? (b.submissionCount - b.answeredSubmissionCount) - (a.submissionCount - a.answeredSubmissionCount) || b.interestedCount - a.interestedCount : b.interestedCount - a.interestedCount);
  ...
  return <section className="questions-board">
    ...
    {result && <div className="q-result" role="status"><Check size={20} /><div><strong>{result.merged ? '같은 요청에 모였어요.' : '새 요청이 등록됐어요.'}</strong><p>{result.question.answer ? '이미 등록된 공통 안내를 아래에서 확인하세요.' : '답변이 등록되면 이 페이지에서 함께 확인할 수 있어요.'}{result.dedupMethod === 'fallback' && ' AI 중복 확인이 지연되어 별도 요청으로 접수했어요.'}{result.dedupMethod === 'mock' && ' 데모 모드에서는 제목과 내용이 같은 요청만 합칩니다.'}</p></div></div>}
