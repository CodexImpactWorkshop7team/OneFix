...
import { departmentLabels, questionDepartments, type Question, type QuestionDepartment, type QuestionResult, type QuestionSubmission } from '@/types/domain';
...
export default function QuestionsBoard({ admin = false }: { admin?: boolean }) {
  ...
  const [status, setStatus] = useState(admin ? 'waiting' : 'all');
  ...
  const refresh = useCallback(async () => {
    ...
    try { setQuestions((await api<{ questions: Question[] }>('questions')).questions); }
  ...
  return <section className="questions-board">
    <div className="page-heading"><div><div className="eyebrow">ONE QUESTION, SHARED ANSWER</div><h1>{admin ? '한 번의 답변으로, 모두에게.' : '같은 궁금증을, 하나로.'}</h1><p>{admin ? '많이 기다리는 요청부터 확인하고 공통 답변을 남겨 주세요.' : '학사부터 생활행정까지. 함께 묻고, 담당 부서의 답변을 함께 확인해요.'}</p></div><div className="q-actions"><button className="button secondary" onClick={() => void refresh()} disabled={loading} aria-label="요청 새로고침"><RefreshCw size={17} className={loading ? 'spin' : ''} /></button>{!admin && <button className="button primary" onClick={() => { setForm(!form); setResult(null); }}><Plus size={17} />요청 등록</button>}</div></div>
    <div className="q-summary"><div><strong>{questions.filter(q => !q.answer).length}</strong><span>답변을 기다리는 요청</span></div><div><strong>{questions.filter(q => q.answer).length}</strong><span>함께 보는 답변</span></div><div className="q-summary-note"><MessageCircle size={23} /><span>같은 부서 · 같은 적용 시기<br /><b>한 답변으로 해결되는 요청만 모아요</b></span></div></div>
    ...
    {result && <div className="q-result" role="status"><Check size={20} /><div><strong>{result.merged ? '같은 요청에 모였어요.' : '새 요청이 등록됐어요.'}</strong><p>{result.question.answer ? '이미 등록된 공통 답변을 아래에서 확인하세요.' : '답변이 등록되면 이 페이지에서 함께 확인할 수 있어요.'}{result.dedupMethod === 'fallback' && ' AI 중복 확인이 지연되어 별도 요청으로 접수했어요.'}{result.dedupMethod === 'mock' && ' 데모 모드에서는 제목과 내용이 같은 요청만 합칩니다.'}</p></div></div>}
    {error && <div className="notice" role="alert">{error}</div>}
    {selectedQuestion && <div className="q-selected"><div className="section-heading"><h2>선택한 요청</h2><button className="text-button" onClick={() => select('')}>닫기 <X size={15} /></button></div><QuestionCard key={`${selectedQuestion.id}-${selectedQuestion.updatedAt}`} question={selectedQuestion} admin={admin} expanded onChange={refresh} /></div>}
    ...
    <div className="section-heading"><div className="tabs">{[['all', '전체'], ['waiting', '답변 대기'], ['answered', '답변 완료']].map(([value, label]) => <button key={value} className={status === value ? 'selected' : ''} onClick={() => setStatus(value)}>{label}</button>)}</div><span className="sort-caption">궁금한 사람 수 ↓</span></div>
    <div className="q-list">{filtered.map((q, i) => <QuestionCard key={`${q.id}-${q.updatedAt}`} question={q} rank={i + 1} admin={admin} onChange={refresh} onSelect={() => select(q.id)} />)}{!loading && !filtered.length && <div className="panel empty-state"><MessageCircle size={32} /><h3>{questions.length ? '조건에 맞는 요청이 없어요' : '첫 번째 궁금증을 알려 주세요'}</h3><p>예: 수강 정정 기간, 장학금 신청 방법, 증명서 발급 안내</p>{!admin && <button className="button primary" onClick={() => setForm(true)}>요청 등록하기<Plus size={17} /></button>}</div>}{loading && !questions.length && <p className="muted">요청을 불러오는 중…</p>}</div>
    <p className="q-footnote">요청 작성자와 ‘나도 궁금해요’ 참여자를 합쳐 요청별 브라우저당 1명으로 집계합니다. 답변은 담당자가 직접 작성하며, 새로고침으로 최신 답변을 확인할 수 있어요.</p>
  </section>;
}
...
function QuestionCard({ question: q, rank, admin, expanded = false, onChange, onSelect }: { question: Question; rank?: number; admin: boolean; expanded?: boolean; onChange: () => Promise<void>; onSelect?: () => void }) {
  const [open, setOpen] = useState(expanded);
  const [submissions, setSubmissions] = useState<QuestionSubmission[] | null>(null);
  const [answer, setAnswer] = useState(q.answer || '');
  const [error, setError] = useState(''); const [busy, setBusy] = useState(false);
  useEffect(() => {
    if (!open || !admin) return;
    let active = true;
    api<{ submissions: QuestionSubmission[] }>(`admin/questions/${q.id}`).then(r => { if (active) setSubmissions(r.submissions); }).catch(e => { if (active) setError(message(e)); });
    return () => { active = false; };
  }, [open, admin, q.id, q.submissionCount]);
  ...
  return <article className="panel q-card"><div className="q-card-heading">{rank && <span className="rank">{String(rank).padStart(2, '0')}</span>}<div className="q-card-main"><div className="q-meta"><span>{departmentLabels[q.department]}</span><span>{q.period}</span><span className={`badge ${q.answer ? 'status-resolved' : 'status-reported'}`}>{q.answer ? '답변 완료' : '답변 대기'}</span></div><button className="q-title" aria-expanded={open} onClick={() => setOpen(!open)}>{q.title}<ChevronDown size={18} className={open ? 'rotated' : ''} /></button><div className="q-card-counts"><span><Users size={14} />{q.interestedCount}명이 궁금해요</span><span>요청 {q.submissionCount}건 취합</span></div></div>{!admin && <button className={`button ${q.hasParticipated ? 'secondary' : 'primary'} q-vote`} disabled={busy || q.hasParticipated} onClick={() => void participate()}>{q.hasParticipated ? <Check size={16} /> : <Plus size={16} />}{q.hasParticipated ? '함께 궁금해요' : '나도 궁금해요'}</button>}</div>
    {open && <div className="q-detail"><p className="q-body">{q.description}</p><div className="q-detail-meta"><span>{date(q.createdAt)} 접수</span>{onSelect && <button className="text-button" onClick={onSelect}>요청 링크 열기<ArrowRight size={14} /></button>}</div>{q.answer ? <div className="q-answer"><div><Check size={18} /><strong>{departmentLabels[q.department]} 공통 답변</strong></div><p>{q.answer}</p><small>{date(q.answeredAt!)} 업데이트 · 취합된 모든 요청에 같은 답변이 표시됩니다.</small></div> : <div className="q-waiting"><MessageCircle size={19} />담당 부서의 답변을 기다리고 있어요.</div>}{admin && <><details className="q-originals"><summary>취합된 원문 {q.submissionCount}건 확인</summary>{submissions ? submissions.map(s => <div key={s.id}><strong>{s.title}</strong><p>{s.description}</p><small>{date(s.createdAt)} · {s.dedupMethod === 'ai' ? 'AI 중복 확인' : s.dedupMethod === 'fallback' ? '중복 확인 없이 접수' : '일반 접수'}</small></div>) : <p>원문을 불러오는 중…</p>}</details><form className="q-answer-form" onSubmit={save}><label htmlFor={`answer-${q.id}-${rank || 'selected'}`}>{q.answer ? '공통 답변 수정' : '공통 답변 작성'}</label><textarea id={`answer-${q.id}-${rank || 'selected'}`} required minLength={5} maxLength={5000} rows={5} value={answer} onChange={e => setAnswer(e.target.value)} placeholder="관련 규정, 신청 방법, 일정과 참고 링크를 적어 주세요." disabled={busy} /><p className="muted small">저장하면 이 요청을 함께 확인하는 {q.interestedCount}명에게 공통으로 표시됩니다.</p><button className="button primary" disabled={busy || answer.trim().length < 5 || answer.trim() === q.answer}><Send size={16} />{busy ? '저장 중…' : '공통 답변 게시'}</button></form></>}</div>}{error && <div className="notice" role="alert">{error}</div>}</article>;
}
