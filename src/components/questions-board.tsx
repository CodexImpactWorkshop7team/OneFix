...
export default function QuestionsBoard({ admin = false }: { admin?: boolean }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try { setQuestions((await api<{ questions: Question[] }>(admin ? 'admin/questions' : 'questions')).questions); setGeneration(n => n + 1); }
  ...
  const filtered = questions.filter(q => (department === 'all' || department === q.department) &&
    (status === 'all' || (status === 'review' ? q.submissionCount > (q.answeredSubmissionCount ?? 0) : status === 'answered' ? Boolean(q.answer) : !q.answer)) &&
    `${q.title} ${q.description} ${q.period} ${q.answer || ''}`.toLocaleLowerCase().includes(search.trim().toLocaleLowerCase()))
    .sort((a, b) => sort === 'newest' ? b.createdAt.localeCompare(a.createdAt) : sort === 'review' ? (b.submissionCount - (b.answeredSubmissionCount ?? 0)) - (a.submissionCount - (a.answeredSubmissionCount ?? 0)) || b.interestedCount - a.interestedCount : b.interestedCount - a.interestedCount);
  ...
  return <section className="questions-board">
    ...
    {admin && <div className="q-page-links"><Link className="text-link" href={admin ? '/questions' : '/admin/questions'}>{admin ? '행정 요청 게시판' : '담당자 답변 관리'}<ArrowRight size={15} /></Link>{admin && <Link className="text-link" href="/admin">설비 요청 관리<ArrowRight size={15} /></Link>}</div>}
    ...
    {result && <div className="q-result" role="status"><Check size={20} /><div><strong>{result.merged ? '같은 요청에 모였어요.' : '새 요청이 등록됐어요.'}</strong><p>{result.question.answer ? '이미 등록된 공통 안내를 아래에서 확인하세요.' : '답변이 등록되면 이 페이지에서 함께 확인할 수 있어요.'}</p></div></div>}
    ...
    <div className="section-heading"><div className="tabs">{[['all', '전체'], ['waiting', '공통 안내 대기'], ['answered', '공통 안내 등록'], ...(admin ? [['review', '개별 검토 필요']] : [])].map(([value, label]) => <button key={value} className={status === value ? 'selected' : ''} onClick={() => setStatus(value)}>{label}</button>)}</div><select className="q-sort" aria-label="요청 정렬" value={sort} onChange={e => setSort(e.target.value)}><option value="popular">궁금한 사람 많은 순</option><option value="newest">최근 접수 순</option>{admin && <option value="review">개별 검토 많은 순</option>}</select></div>
    ...
    <p className="q-footnote">요청 작성자와 ‘나도 궁금해요’ 참여자를 합쳐 요청별 브라우저당 1명으로 집계합니다. 담당자가 등록한 답변과 최신 처리 상황을 이 페이지에서 확인할 수 있어요.</p>
  </section>;
}
