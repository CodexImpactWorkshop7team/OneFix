...
function SubmissionAnswer({ question, item, number, admin, onSaved }: { question: Question; item: QuestionSubmission; number: number; admin: boolean; onSaved: (item: QuestionSubmission) => void }) {
  ...
  async function save(e: FormEvent) {
    ...
    try {
      const r = await api<{ submission: QuestionSubmission }>(base, { method: 'PATCH', body: JSON.stringify({ answer, status, revision: item.answerRevision, sourceUpdatedAt: draft?.sourceUpdatedAt || sourceUpdatedAt }) });
      onSaved(r.submission);
    } catch (e) { setError(e instanceof Error ? e.message : '답변을 저장하지 못했어요.'); }
    finally { setSaving(false); }
  }
  const label = item.answerStatus === 'answered' ? '개별 답변 등록' : item.answerStatus === 'needs_clarification' ? '추가 확인 요청' : '개별 검토 대기';
