...
export function predictions(): Predictions {
  const t = midnight();
  const items: PredictionItem[] = facilities().map(facility => {
    const observed = all<{ observed_from: string }>('SELECT observed_from FROM facilities WHERE id=?', facility.id)[0].observed_from;
    ...
    const records = all<{ at: string }>(`SELECT MIN(r.created_at) AS at FROM reports r JOIN issues i ON i.id=r.issue_id
    ...
    const k = all<{ n: number }>('SELECT COUNT(*) AS n FROM issues WHERE facility_id=? AND created_at>=? AND created_at<?', facility.id, iso(t - 30 * DAY), iso(t))[0].n;
    const open = all<{ n: number }>("SELECT COUNT(*) AS n FROM issues WHERE facility_id=? AND status<>'resolved'", facility.id)[0].n;
  ...
  return { asOf: iso(t), asOfDate: iso(t + KST).slice(0, 10), generatedAt: iso(Date.now()), timezone: 'Asia/Seoul', methodVersion: 'history-v1', datasetKind: datasetKind(), items };
}
