...
import { z } from 'zod';
import { all, one, run, transaction, iso } from './db';
...
async function list(clientId: string): Promise<Question[]> {
  return (await all<Question>(`SELECT q.id,q.department,q.period,q.title,q.description,q.answer,
...
async function get(id: string, clientId: string) {
  const q = (await list(clientId)).find(q => q.id === id);
  if (!q) throw new ApiError(404, 'QUESTION_NOT_FOUND', '요청을 찾을 수 없어요.');
  return q;
}
async function submissions(questionId: string): Promise<QuestionSubmission[]> {
  return (await all<QuestionSubmission>(`SELECT s.id,s.title,s.description,s.created_at AS createdAt,s.dedup_method AS dedupMethod,
...
async function submission(questionId: string, id: string) {
  const item = (await submissions(questionId)).find(s => s.id === id);
...
export async function handleQuestions(req: Request, segments: string[], clientId: string): Promise<Response | null> {
  ...
  if (!admin && req.method === 'POST' && parts.length === 1) {
    ...
    return serialize(`question:${input.department}:${input.period}`, async () => {
      ...
      const question = (await transaction(async () => {
        const id = match.id || randomUUID(), now = iso(Date.now());
        if (!match.id) (await run('INSERT INTO questions VALUES (?,?,?,?,?,NULL,NULL,?,?)', id, input.department, input.period, input.title, input.description, now, now));
        (await run('INSERT INTO question_submissions VALUES (?,?,?,?,?,?,?,?,?)', randomUUID(), id, input.requestId, hash, input.title, input.description, match.method, match.id ? 1 : 0, now));
  ...
  if (admin && req.method === 'POST' && parts.length === 5 && parts[2] === 'submissions' && parts[4] === 'suggestion') {
    return serialize(`answer-suggestion:${parts[3]}`, async () => {
      const question = (await get(parts[1], clientId));
      const draft = await suggestAnswer(question, (await submission(question.id, parts[3])));
      if ((await get(question.id, clientId)).updatedAt !== draft.sourceUpdatedAt) throw new ApiError(409, 'ANSWER_CHANGED', '추천 중 공통 답변이 변경됐어요. 새로고침 후 다시 추천해 주세요.');
  ...
  if (admin && req.method === 'PATCH' && parts.length === 4 && parts[2] === 'submissions') {
    ...
    return json((await transaction(async () => {
      const question = (await get(parts[1], clientId)), item = (await submission(question.id, parts[3]));
  ...
  if (admin && req.method === 'PATCH' && parts.length === 2) {
    ...
    return json((await transaction(async () => {
      const current = (await get(parts[1], clientId));
      if (current.updatedAt !== input.updatedAt) throw new ApiError(409, 'ANSWER_CHANGED', '다른 답변이 저장되었어요. 새로고침 후 확인해 주세요.');
      const now = iso(Date.now());
      (await run('UPDATE questions SET answer=?,answered_at=?,updated_at=? WHERE id=?', input.answer, now, now, current.id));
      return { question: (await get(current.id, clientId)) };
    })));
  }
