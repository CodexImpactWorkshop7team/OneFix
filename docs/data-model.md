...
첫 28일 구간 이전부터 이어진 고장에 첫 주 신고를 연결할 수 있게 프린터·에어컨에 T−35일 고장 각 1건을 추가한다. 새로운 고장이 열리기 전날 이전 고장을 해결해 과거 신고가 아직 생성되지 않은 고장에 연결되지 않게 한다.
...
시드 실행은 예측 집계로 주별 건수, 미해결 영향 인원, 주의도를 검증해 불일치하면 실패해야 한다. 합성 이력은 실제 운영 지표·정확도 평가에 사용하지 않는다.
