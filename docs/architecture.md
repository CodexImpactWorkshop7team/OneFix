...
| 저장소 | `@libsql/client`, 비동기 SQL | 로컬 SQLite / Vercel Turso libSQL 공용 |
...
로그는 요청 ID, 처리 시간, 결과 모드, 새 고장/병합 여부, 오류 코드만 기록한다. 관리자 인증·API 권한, 로그인 시도 제한, DB 사진 저장과 공유 접수 잠금을 적용했다. 운영용 공개 접수 스팸 방지 및 데이터 보존 정책은 추가해야 한다.

최신 배포·인증 구조는 [Vercel 배포 가이드](deployment.md)를 따른다. 초기 구현의 동기 SQL·파일 사진 저장 설명은 로컬 초기 버전 기록이다.
